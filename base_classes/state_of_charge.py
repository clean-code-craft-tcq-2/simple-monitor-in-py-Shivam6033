...
class StateOfCharge(RangeService):
    ...
    def print_out_of_range_message(self):
        print(f'State Of Charge {self.language.get_out_of_range_message()}')
    ...
    def warn_if_within_warning_tolerance(self):
        ...
        if self.value in range(self.lower_limit, lower_limit_tolerance + 1):
            print(f'State Of Charge {self.language.get_approaching_discharge_message()}')
        if self.value in range(higher_limit_tolerance, self.higher_limit):
            print(f'State Of Charge {self.language.get_approaching_charge_peak_message()}')
